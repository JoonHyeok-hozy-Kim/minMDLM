...
def generate_sudoku_rare(num_grids, transform_cnt=5):
    # Setup for saving samples
    dataset_dir = os.path.join(PROJECT_ROOT, "tests", "sudoku", "dataset")
    os.makedirs(dataset_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M")
    file_name = f"sudoku_rare_dataset_size_{num_grids}_{timestamp}.txt"
    file_to_write = os.path.join(dataset_dir, file_name)
    
    # Create Sudoku
    S = SudokuGrid()
    S.create_sudoku_grids(num_grids=num_grids, shuffle_range=True, transform_cnt=transform_cnt, shuffle_coord=True)
